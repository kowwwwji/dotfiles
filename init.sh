...
# statusline から呼ぶモデル個別の週次上限の取得スクリプト
ln -nfs "${DOTFILES_ROOT}/dot_claude/usage-limits.sh" "${HOME}/.claude/usage-limits.sh"
# 旧構成（ローカル集計で按分する model-share.sh を張っていた）の名残はダングリング
# リンクになるため掃除する。キャッシュも残骸になるので併せて消す。
[[ -L "${HOME}/.claude/model-share.sh" ]] && rm "${HOME}/.claude/model-share.sh"
rm -f "${HOME}/.claude/cache/model-share.json"

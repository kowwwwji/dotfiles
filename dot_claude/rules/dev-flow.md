...
- **まとまった実装**（目安: スペック・計画が文書化されていて独立実行できる、
  または並行して進めたい作業）→ **tmux 別 window の新セッションへ委譲する**:
  ...
  - **Why**: メインの会話と window をユーザーの手元に残したまま実装を並行・可視化
    できる。進捗は bell / 通知で把握できる。
- **中規模**（worktree は要るが、対話しながら進める方が速い作業）→ 同一セッションで
  EnterWorktree（path 指定。worktree.md 参照）。

...
# vendor: sketchybar-app-font (https://github.com/kvndrsslr/sketchybar-app-font) の
# GitHub Release v2.0.69 の icon_map.sh のコピー（Brewfile.common の
# font-sketchybar-app-font と同バージョンを使う。リガチャとマップの対応が
# バージョン間でずれると豆腐になるため）。このヘッダー以外は無改変・実行ビットのみ付与。
# font 更新時は対応 Release の icon_map.sh を再取得して上書きし、このヘッダーを付け直す。

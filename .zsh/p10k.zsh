# Generated by Powerlevel10k configuration wizard on 2021-11-09 at 15:55 JST.
...
function prompt_git_worktree() {
  ...
  # NOTE: 使用フォント（Maple Mono NF CN 等 Nerd Fonts v3 系）に合わせて nerdfont-v3。
  # nerdfont-complete(v2) だと旧MDI領域(U+F500-FD46)のアイコンがグリフ欠落で
  # wezterm が "No fonts contain glyphs" 警告を出す（例: gcloud の U+F7B7）。
  ...
  # gcs で stg/prod を切り替える運用のため、gcloud と同様に常時表示にする（kubectl 入力中だけでは向き先の確認が遅れる）
  ...
  typeset -g POWERLEVEL9K_KUBECONTEXT_CLASSES=(
      '*prod*'  PROD    # 本番クラスタを向いているときは色で警告する
      '*'       DEFAULT)
  typeset -g POWERLEVEL9K_KUBECONTEXT_DEFAULT_FOREGROUND=134
  typeset -g POWERLEVEL9K_KUBECONTEXT_PROD_FOREGROUND=160  # status のエラー表示と同じ赤
  ...
  # PROD も同じ短縮表示にする（クラス別の CONTENT_EXPANSION が無いとフルのコンテキスト名に戻る）
